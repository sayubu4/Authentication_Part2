...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($cat_id) || empty($brand_id) || empty($region_id) || empty($title) || empty($price) || empty($desc) || empty($keywords)) {
    ...
    try {
        $result = add_product_ctr($cat_id, $brand_id, $title, $price, $desc, $image_path, $keywords, $region_id, $opening_hours, $contact_phone, $exact_location);
        ob_clean();
        if ($result) {
